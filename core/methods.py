...
# First party imports
from .HelperTools import timer as ht_timer
...
def preprop_lstat(dfr, dfg, pdict):
    ...
    dframe3["PLZ"] = dframe3["PLZ"].fillna(0).astype(int)
    dframe3 = dframe3.drop_duplicates()
...
def count_plz_occurrences(df_lstat2):
    ...
    result_df = (
        df_lstat2.groupby("PLZ")
        .agg(Number=("PLZ", "count"), geometry=("geometry", "first"))
        .reset_index()
    )
...
# -----------------------------------------------------------------------------
@ht_timer
def make_streamlit_electric_Charging_resid(dfr1, dfr2, gdfr1, gdfr2):
    ...
    layer_selection = st.radio(
        "Select Layer",
        (
            "Number of Residents per PLZ (Postal code)",
            "Number of Charging Stations per PLZ (Postal code)",
        ),
    )
